...
function MongoStore(connection, options) {
  ...
  this._options.useUnifiedTopology = true;
  this._collectionName = "passwordless-token";
  this._dbName = this._collectionName;
  if (this._options.mongostore) {
    if (this._options.mongostore.collection) {
      ...
      this._dbName = this._options.mongostore.collection;
    }
...
MongoStore.prototype.invalidateUser = function (uid, callback) {
  ...
  this._get_collection(function (collection) {
    collection.deleteOne({ uid: uid }, { w: 1 }, function (err, result) {
      if (err) {
        callback(err);
      } else {
        callback();
...
MongoStore.prototype.clear = function (callback) {
  ...
  this._get_collection(function (collection) {
    collection.removeMany({}, { w: 1 }, function (err, result) {
      if (err) {
        callback(err);
      } else {
        callback();
  ...
  if (self._db) {
    ...
    MongoClient.connect(self._uri, self._options, function (err, client) {
      if (err) {
        ...
        self._db = client.db(self._dbName);
        callback(client.db(self._dbName));
      }
    });
  }
};
...
MongoStore.prototype._get_collection = function (callback) {
  ...
  if (self._collection) {
    ...
    self._connect(function (db) {
      const collection = db.collection(self._collectionName);
